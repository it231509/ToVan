...
<template>
  <div class="max-w-2xl mx-auto pt-2">
    ...
    <div class="space-y-6">
      <div class="bg-base-100 p-4 rounded-3xl border border-base-300 shadow-sm flex items-center gap-4">
        <div class="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center text-primary text-2xl font-black">
          {{ userEmail.charAt(0).toUpperCase() }}
        </div>
        <div>
          <h1 class="font-bold text-base-content/60 uppercase text-xs tracking-widest">Eingeloggt als</h1>
      ...
      <div class="bg-base-100 rounded-3xl border border-base-300 shadow-sm overflow-hidden">
        <div class="p-4 border-b border-base-200 flex justify-between items-center">
          <div>
            ...
            <p class="text-base-content/50" style="font-size: 14px;">Design der App anpassen</p>
          </div>
          <input 
            type="checkbox" 
        ...
        <button 
          @click="handleLogout"
          class="w-full p-4 flex items-center gap-4 hover:bg-error/5 transition-colors group"
        >
